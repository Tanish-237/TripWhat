...
import { MapPin, DollarSign, Users, Sparkles, CheckCircle } from "lucide-react";
...
const TripPlanningSidebar = ({ currentStep, onStepClick, tripData }) => {
  const isBudgetValid = (budget) => {
    ...
    const hasValidAllocations = categories.every(
      (cat) => budget[cat] !== undefined && budget[cat] >= 0
    );
    if (!hasValidAllocations) return false;
    const totalAllocations = categories.reduce(
      (sum, cat) => sum + (budget[cat] || 0),
      0
    );
    const looksLikePercentages =
      categories.every((cat) => budget[cat] <= 100) &&
  ...
  const getStepStatus = (stepId) => {
    ...
    switch (stepId) {
      ...
      case "preferences":
        return tripData?.people && tripData?.travelType
          ? "completed"
          : "pending";
      case "results":
  ...
  return (
    <div className="w-80 bg-white border-r border-gray-200 min-h-screen">
      <div className="sticky top-4 p-5 max-h-[calc(100vh-2rem)] overflow-y-auto">
        ...
        (tripData.cities?.length > 0 ||
          tripData.startDate ||
          (tripData.people && tripData.people > 0) ||
              ...
              <div className="space-y-2 text-xs">
                        ...
                        <span className="font-medium text-blue-900">
                          {new Date(
                            new Date(tripData.startDate).getTime() +
                              (tripData.totalDays - 1) * 24 * 60 * 60 * 1000
                ...
                {tripData?.people > 0 && (
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-blue-900">
                      {tripData.people}{" "}
                      {tripData.people === 1 ? "person" : "people"}
                    </span>
                  </div>
                )}
          ...
          <Card className="bg-gray-50 border-gray-200">
            <div className="p-4 text-center">
              ...
              <p className="text-xs text-gray-500">
                Add destinations to see your trip summary
              </p>
            </div>
          </Card>
        )}
